...
\Rfunction{getTaxa}      & Get a unique vector of the observed taxa at a particular taxonomic rank \\
\Rfunction{getVariable}     & Returns an individual sample variable vector/factor \\
...
\Rfunction{rank.names}      & Get the names of the available taxonomic ranks \\
...
\Rfunction{sample.variables}& Returns the names of sample variables in an object \\
