...
ggsave("phyloseq_analysis-GPCAspecplot.pdf", p1, width=10, height=9)
@
\begin{figure}[htbp]
\centering
\includegraphics[width=5in]{phyloseq_analysis-GPCAspecplot}
\caption{Species plot of the ``Global Patterns'' correspondence analysis first two axes, with each phylum on a  different panel (``facet''). Only the phyla with at least one million individuals observed (cumulative, all samples) are included.}
...
Here is a quick example illustrating the use of Double Principal Coordinate Analysis (DPCoA~\cite{Pavoine2004523}), using the using the \code{DPCoA()} function in phyloseq, as well as the ``biplot'' option for \code{plot{\_}ordination()}.
...
<<GPdpcoa00, fig=FALSE, include=FALSE>>=
topsp <- names(sort(speciesSums(GP), TRUE)[1:200])
GP1   <- prune_species(topsp, GP)
@

Now perform the DPCoA on the simplified dataset, and plot.

<<GPdpcoa01, fig=FALSE>>=
GP.dpcoa <- DPCoA(GP1)
( pdpcoa <- plot_ordination(GP1, GP.dpcoa, type="biplot",
     color="SampleType", shape="Phylum") )
@

<<GPdpcoa02, echo=FALSE, fig=FALSE, include=FALSE>>=
ggsave("phyloseq_analysis-GPdpcoaBiplot.pdf", pdpcoa, width=9, height=7)
@
...
\code{distance()} currently supports 43 method options, as well as user-provided arbitrary methods via an interface to vegan's \code{designdist()} function. Currrently only sample-wise distances are supported (the \code{type} argument), but eventually species-wise (OTU-wise) distances will be supported as well. 

In addition to supporting any of the method options for the three distance functions in the \code{vegan}-package, \code{distance()} will eventually support many of the distance methods in the \code{ade4}-package. It also supports  \code{UniFrac} distance function (Section~\ref{sec:unifrac}), a method for calculating Double Principal Coordinate Analysis (DPCoA), as well as an extension to the vegan interface for arbitrary, user-defined distances.
