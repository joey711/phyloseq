...
QIIME is a free, open-source OTU clustering and analysis pipeline written for Unix (mostly Linux)~\cite{Caporaso:2010jf}. It is distributed in a number of different forms (including a pre-installed virtual machine). See [the QIIME home page](http://qiime.org/) for details.
...
The open-source, platform-independent, locally-installed software package, *mothur*, can also process barcoded amplicon sequences and perform OTU-clustering~\cite{Schloss:2009do}. It is extensively documented on [the mothur wiki](http://www.mothur.org/wiki/)
...
```{r build-component-data-table, echo=FALSE, message=FALSE, results='asis'}
L = list(
  c("Function      "," Input Class      "," Output Description"),
  c("`otu_table` "," numeric matrix   "," `otu_table` object storing OTU abundance"),
  c("`otu_table` "," data.frame"," `otu_table` object storing OTU abundance"),
  c("`sample_data`"," data.frame"," `sample_data` object storing sample variables"),
  c("`tax_table`   "," character matrix "," `taxonomyTable` object storing taxonomic identities"),
  c("`tax_table`   "," data.frame "," `taxonomyTable` object storing taxonomic identities"),
  c("`read_tree`      "," file path char   "," phylo-class tree, read from file"),
  c("`read.table`      "," table file path  "," A matrix or data.frame (Std `R` core function) ")
)
L = lapply(L, FUN=gsub, pattern="^[[:space:]]+", replacement="\\2")
L = lapply(L, FUN=gsub, pattern="[[:space:]]+$", replacement="\\2")
require("plyr")
require("xtable")
print(xtable(ldply(L)), type="html")
```

Functions for building complex data objects

```{r build-complex-table, echo=FALSE, message=FALSE, results='asis'}
L = list(
  c("Two or more component objects", "phyloseq-class, *experiment-level* object"),
  c("Two or more component or phyloseq-class objects ", " Combined instance of phyloseq-class")
)
L = lapply(L, FUN=gsub, pattern="^[[:space:]]+", replacement="\\2")
L = lapply(L, FUN=gsub, pattern="[[:space:]]+$", replacement="\\2")
Ltab = ldply(L)
colnames(Ltab) <- c("Input Class", "Output Description")
rownames(Ltab) <- c("`phyloseq`", "`merge_phyloseq`")
require("plyr")
require("xtable")
print(xtable(Ltab), align=c("ll"), type="html")
```
...
```{r accessor-xtable, echo=FALSE, message=FALSE, results='asis'}
L = list(
  c("Function                    "," Returns"),
  c("`[` "," Standard extraction operator. Works on `otu_table`, `sample_data`, and `taxonomyTable`"),
  c("`access`   "," General slot accessor function for phyloseq-package"),
  c("`get_taxa`      "," Abundance values of all taxa in sample `i'"),
  c("`get_sample`      "," Abundance values of taxa `i' for all samples"),
  c("`get_taxa_unique`      "," A unique vector of the observed taxa at a particular taxonomic rank"),
  c("`get_variable`     "," An individual sample variable vector/factor"),
  c("`nsamples` "," Get the number of samples described by an object"),
  c("`ntaxa` "," Get the number of OTUs (taxa) described by an object"),
  c("`otu_table`", "Build or access otu_table objects"),
  c("`rank_names`      "," Get the names of the available taxonomic ranks"),
  c("`sample_data`      "," Build or access `sample_data` objects"),
  c("`sample_names`    "," The names of all samples"),
  c("`taxa_names`   "," The names of all taxa"),
  c("`sample_sums`      "," The sum of the abundance values of each sample"),
  c("`sample_variables` "," The names of sample variables"),
  c("`taxa_sums`     "," The sum of the abundance values of each taxa"),
  c("`taxa_are_rows`  "," `TRUE` if taxa are row indices in `otu_table`"),
  c("`tax_table`   "," A taxonomy table"),
  c("`phy_tree`    "," Access the tree contained in a phyloseq object")
)
L = lapply(L, FUN=gsub, pattern="^[[:space:]]+", replacement="\\2")
L = lapply(L, FUN=gsub, pattern="[[:space:]]+$", replacement="\\2")
require("plyr")
...
topN <- 20
```
For example, lets make a new object that only holds the most abundant \Sexpr{topN} taxa in the experiment. To accomplish this, we will use the `prune_taxa()` function.
```{r}
data(GlobalPatterns)
