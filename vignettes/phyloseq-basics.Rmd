...
Paul J. M{c}Murdie and Susan Holmes
...
---
# Load *phyloseq* and import data<a id="pookie"></a>sec:load}
## Load *phyloseq*
...
## Import from biom-format}<a id="pookie"></a>sec:biom}
...
## Import from QIIME}<a id="pookie"></a>sec:qiimeimport}
...
<a id="pookie"></a>fig:qiimedirectory}
...
---
## Import from mothur}<a id="pookie"></a>sec:mothurimport}
...
### Output
`import_RDP_tab()` returns an instance of the phyloseq-class
...
so analyses like `tip_glom()` and `UniFrac` are not applicable.
...
Suppose you have already imported raw data from an experiment into `R}, and their indices are labeled correctly. How do you get *phyloseq* to recognize these tables as the appropriate class of data? And further combine them together? Table~\ref{table:build} lists key functions for converting these core data formats into specific component data objects recognized by *phyloseq*. These will also
...
\hline
\Rfunction{`otu_table`        & numeric matrix   & `otu_table` object storing OTU abundance \\
\Rfunction{`otu_table`        & data.frame       & `otu_table` object storing OTU abundance \\
\Rfunction{`sample_data`       & data.frame       & `sample_data` object storing sample variables \\
\Rfunction{tax_}table}          & character matrix & `taxonomyTable` object storing taxonomic identities \\
\Rfunction{tax_}table}          & data.frame & `taxonomyTable` object storing taxonomic identities \\
\Rfunction{read_}tree}             & file path char   & phylo-class tree, read from file \\
...
\Rfunction{merge_}phyloseq}  & 2 or more component or phyloseq-class objects & Combined instance of phyloseq-class \\
\hline
\end{tabular}

###### { Constructors: functions for building *phyloseq* objects.}
<a id="pookie"></a>table:build}
\end{table}
...
You do not need to have all four data types in the example above in order to combine them into one validity-checked experiment-level phyloseq-class object. The `phyloseq()` method will detect which component data classes are present, and build accordingly. Downstream analysis methods will access the required components using emph{phyloseq}'s accessors, and throw an error if something is missing. For most downstream methods you will only need to supply the combined, phyloseq-class object (the output of `phyloseq()` ), usually as the first argument. 
```{r, eval=FALSE}
...
\item Merging the OTUs or samples in a phyloseq object}, based upon a taxonomic or sample variable:

merge\_{}samples() merge\_{}taxa()

\item Merging two or more data objects} that come from the same experiment, so that their data becomes part of the same phyloseq object:
...
---
# Accessor functions}<a id="pookie"></a>sec:accessors}
...
\Rfunction{[}               & Standard extraction operator.
          works on `otu_table`, `sample_data`, and `taxonomyTable` \\
\Rfunction{access}          & General slot accessor function for phyloseq-package \\
\Rfunction{get_}taxa}      & Abundance values of all taxa in sample `i' \\
\Rfunction{get_}sample}      & Abundance values of taxa `i' for all samples \\
\Rfunction{get_}taxa_}unique}      & A unique vector of the observed taxa at a particular taxonomic rank \\
\Rfunction{get_}variable}     & An individual sample variable vector/factor \\
...
\Rfunction{`otu_table`        & Build or access otu_}table objects \\
\Rfunction{rank_}names}      & Get the names of the available taxonomic ranks \\
\Rfunction{`sample_data`      & Build or access `sample_data` objects \\
\Rfunction{sample_}names}    & The names of all samples \\
\Rfunction{taxa_}names}   & The names of all taxa \\
\Rfunction{sample_}sums}      & The sum of the abundance values of each sample \\
\Rfunction{sample_}variables} & The names of sample variables \\
\Rfunction{taxa_}sums}     & The sum of the abundance values of each taxa \\
\Rfunction{taxa_}are_}rows}  & `TRUE} if taxa are row indices in `otu_table` \\
\Rfunction{tax_}table}          & A taxonomy table \\
\Rfunction{tre}             & Access the tree contained in a phyloseq object \\
\hline
\end{tabular}

###### { Accessor functions for *phyloseq* objects.}
<a id="pookie"></a>table:access}
\end{table}
...
---
# Trimming, subsetting, filtering phyloseq data<a id="pookie"></a>sec:trim}
## Trimming: `prune_}taxa()} and `prune_}samples()}}
...
In general, most trimming should be accomplished using the S4 methods `prune_}taxa()} or `prune_}samples()}.

## Simple filtering example}
```{r echo=FALSE}
topN <- 20
```
For example, lets make a new object that only holds the most abundant \Sexpr{topN} taxa in the experiment. To accomplish this, we will use the `prune_}taxa()} function.
```{r}
data(GlobalPatterns)
...
### `filter_}taxa}: Filter by Across-Sample Criteria}
The `filter_}taxa} function is directly analogous to
the `genefilter} function for microarray filtering,
but is used for filtering OTUs from phyloseq objects.
It applies an arbitrary set of functions ---
as a function list, for instance, created by `genefilter::filterfun} ---
...
Note that the functions `genefilter} and `kOverA} are from
...
## `subset_}samples}: Subset by Sample Variables}
It is possible to subset the samples in a *phyloseq* object based on the sample variables using the `subset_}samples()} function. For example to subset `GlobalPatterns` such that only *Gender A} is present, the following line is needed (the related tables are subsetted automatically as well):
```{r}
...
## `subset_}taxa()}: subset by taxonomic categories}
It is possible to subset by specific taxonomic category using the `subset_}taxa()} function. For example, if we wanted to subset `GlobalPatterns` so that it only contains data regarding the phylum *Firmicutes}:
```{r}
...
---
# Phylogenetic smoothing}<a id="pookie"></a>sec:glom}
##  `tax_glom()` Method}
...
![phyloseq class structure](phyloseq_classes_7.pdf)
...
http://joey711.github.com/phyloseq/install} 

for help with installation. This is likely to be the first place news and updated information about installation will be posted, as well. Also check out the rest of the phyloseq homepage on GitHub 
