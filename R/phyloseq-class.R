...
phyloseq <- function(...) {
  ...
  # rm any forbidden chars in index names (e.g. quotes - phylogenetic tree).  Right
  # now, only extra quotes are forbidden.
  ...
  #################### Need to determine whether to (A) instantiate a new raw/uncleaned phyloseq
  #################### object, or (B) return a single component, or (C) to stop with an error because
  #################### of incorrect argument types.
  ...
  #################### Reconcile the taxa and sample index names between components in the
  #################### newly-minted phyloseq object
  ...
  # Verify there is more than one component that describes samples before
...
################################################################################ A relatively fast way to access from phyloseq object components f - function
################################################################################ name as character string physeq - a phyloseq object (phyloseq-class instance)
#' @keywords internal
...
# f_comp_ps('taxa_names', ps) f_comp_ps('ntaxa', ps) Reduce('union',
# f_comp_ps('taxa_names', ps)) Reduce('intersect', f_comp_ps('taxa_names', ps))
...
get.component.classes <- function() {
  ...
  component.classes <- c("otu_table", "sample_data", "phylo", "taxonomyTable", 
    "XStringSet")
  # the names of component.classes needs to be the slot names to match getSlots /
  # splat
...
# Returns TRUE if x is a component class, FALSE otherwise.  This shows up over
# and over again in data infrastructure
#' @keywords internal
...
splat.phyloseq.objects <- function(x) {
  if (is.component.class(x)) {
    ...
    splatx <- list(x)
    names(splatx) <- names(which(sapply(get.component.classes(), function(cclass, 
      x) inherits(x, cclass), x)))
  } else if (inherits(x, "phyloseq")) {
    # Else, check if it inherits from phyloseq, and if-so splat
    slotnames = names(getSlots("phyloseq"))
    allslots = sapply(slotnames, function(i, x) {
      access(x, i, FALSE)
    }, x)
...
setMethod("index_reorder", "phyloseq", function(ps, index_type = "both") {
  if (index_type %in% c("both", "taxa")) {
    ...
    if (!is.null(phy_tree(ps, FALSE))) {
      # If there is a phylogenetic tree included, re-order based on that, and reorder
      # the otu_table The new taxa order, torder, will also trickle down to the
      # taxonomyTable or XStringSet if present.
  ...
  if (index_type %in% c("both", "samples")) {
    ## ENFORCE CONSISTENT ORDER OF SAMPLE INDICES Errors can creep when sample indices
    ## do not match.
