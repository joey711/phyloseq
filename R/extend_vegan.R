################################################################################ Define S3 methods for scores (originally defined by vegan-package) to work for
################################################################################ other ordination results vegan:::scores.default pcoa-class, from pcoa{ape}
...
scores.pcoa <- function(x, choices = NULL, display = "sites", physeq = NULL, ...) {
  ...
  if ("species" %in% display) {
    if (is.null(otu_table(physeq, errorIfNULL = FALSE))) {
      ...
      # MDS/PCoA only provides coordinates of the elements in the distance matrix,
      # usually sites/samples, so species (etc.)  This means we need to use the
      # weighted-average as there is no corresponding axes from the ordination
      # directly.
...
get_dpcoa_species_coords = function(x, physeq = NULL) {
  ...
  coords = x$dls
  # ade4 mangles the element names using `make.names` conventions in base R Replace
  # them in `coords`
  if (is.null(taxa_names(physeq))) {
    ...
    # if the names are available, use them by mapping the same variable-name
    # conversion that ade4 would have used.
...
scores.dpcoa <- function(x, choices = NULL, display = "sites", physeq = NULL, ...) {
  # x = ordination display = 'species'
  coords = NULL
  # `display` must be either 'sites' or 'species', per vegan-package convention.
  coords <- switch(EXPR = display, species = get_dpcoa_species_coords(x, physeq), 
...
################################################################################ Extend vegdist for phyloseq classes \code{\link[vegan]{vegdist}} wrapper for
################################################################################ phyloseq classes Trivially-extended S4 method from the
################################################################################ \code{\link[vegan]{vegdist}} function, such that S4 classes from the
################################################################################ \code{\link{phyloseq-package}} are properly handled / accessed. All
################################################################################ parameters passed on to \code{\link[vegan]{vegdist}} verbatim.  @seealso
################################################################################ \code{\link[vegan]{vegdist}} @rdname vegdist-methods @docType methods
################################################################################ @aliases vegdist @examples data(esophagus) vegdist(esophagus, 'jaccard')
#' @importFrom vegan vegdist
#' @keywords internal
setGeneric("vegdist")
################################################################################ @aliases vegdist,otu_table-method @rdname vegdist-methods
#' @importFrom vegan vegdist
setMethod("vegdist", "otu_table", function(x, method = "bray", binary = FALSE, diag = FALSE, 
...
################################################################################ @aliases vegdist,phyloseq-method @rdname vegdist-methods
setMethod("vegdist", "phyloseq", function(x, method = "bray", binary = FALSE, diag = FALSE, 
  upper = FALSE, na.rm = FALSE, ...) {
...
estimate_richness <- function(physeq, split = TRUE, measures = NULL) {
  ...
  if (!any(otu_table(physeq) == 1)) {
    # Check for singletons, and then warning if they are missing.  These metrics only
    # really meaningful if singletons are included.
    warning("The data you have provided does not have\n", "any singletons. This is highly suspicious. Results of richness\n", 
      "estimates (for example) are probably unreliable, or wrong, if you have already\n", 
      "trimmed low-abundance taxa from the data.\n", "\n", "We recommended that you find the un-trimmed data and retry.")
  }
  
  # If we are not splitting sample-wise, sum the species. Else, enforce
  # orientation.
  if (!split) {
  ...
  renamevec = c("Observed", "Chao1", "ACE", "Shannon", "Simpson", "InvSimpson", 
    "Fisher")
  names(renamevec) <- c("S.obs", "S.chao1", "S.ACE", "shannon", "simpson", "invsimpson", 
    "fisher")
