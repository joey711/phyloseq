...
distance <- function(physeq, method = "unifrac", type = "samples", ...) {
  ...
  extrargs <- list(...)
  # # non-phyloseq methods are assumed to be based on otu_table-only (for now) # If
  # necessary (non phyloseq funs), enforce orientation, build function.
  OTU <- otu_table(physeq)
  # disambiguate type argument... Must be 'species' for vegan integration...  The
  # following should all work: 'OTUs', 'OTU', 'otus', 'Taxas', 'site'
...
phyloseq_JSD_pair <- function(x, y) {
  ### Function to compute Shannon-Jensen Divergence x and y are the frequencies for
  ### the same p categories
  u <- x/sum(x)
  v <- y/sum(y)
  m <- (u + v)/2
  if (all(u * v > 0)) {
...
JSD <- function(physeq, parallel = FALSE) {
  OTU <- otu_table(physeq)
  ### Some parallel-foreach housekeeping.  If user specifies not-parallel run (the
  ### default), register the sequential 'back-end'
  if (!parallel) {
    registerDoSEQ()
  }
  ...
  ## Format coercion Coerce to the vegan orientation, with species as columns
  ...
  # return(distlist) This is in serial, but it is quick.
...
setMethod("UniFrac", "phyloseq", function(physeq, weighted = FALSE, normalized = TRUE, 
  ...
  if (!is.rooted(phy_tree(physeq))) {
    ...
    phy_tree(physeq) <- root(phy = phy_tree(physeq), outgroup = randoroot, resolve.root = TRUE, 
      interactive = FALSE)
    if (!is.rooted(phy_tree(physeq))) {
...
fastUniFrac <- function(physeq, weighted = FALSE, normalized = TRUE, parallel = FALSE) {
  ...
  OTU <- as(OTU, "matrix")
  # Enforce that tree and otu_table indices are the same order, by re-ordering OTU,
  # if needed
  if (!all(rownames(OTU) == taxa_names(tree))) {
    OTU <- OTU[taxa_names(tree), ]
  }
  ######################################## Build the requisite matrices as defined in the Fast UniFrac article.  This only
  ######################################## needs to happen once in a call to UniFrac.  Notice that A and B do not appear
  ######################################## in this section.  Begin by building the edge descendants matrix
  ######################################## (edge-by-sample) `edge_array` Create a list of descendants, starting from the
  ######################################## first internal node (root)
  descList <- prop.part(tree, check.labels = FALSE)
  # Add the terminal edge descendants (tips). By definition, can only have one
  # descendant
  descList <- c(as.list(1:length(tree$tip.label)), descList)
  # Convert `descList` to `edge_array` that matches the order of things in
  # `tree$edge`
  edge_array <- matrix(0, nrow = nrow(tree$edge), ncol = nsamples(physeq), dimnames = list(NULL, 
  ...
  for (i in 1:nrow(tree$edge)) {
    ...
    edge_array[i, ] <- colSums(OTU[descList[[tree$edge[i, 2]]], , drop = FALSE], 
      na.rm = TRUE)
  }
  ...
  if (weighted & normalized) {
    # This is only relevant to weighted-UniFrac.  For denominator in the normalized
    # distance, we need the age of each tip.  'z' is the tree in postorder order used
    # in calls to .C Descending order of left-hand side of edge (the ancestor to the
    # node)
    z = reorder.phylo(tree, order = "postorder")
    # Call phyloseq-internal function that in-turn calls ape's internal horizontal
    # position function, in C, using the re-ordered phylo object, `z`
    tipAges = ape_node_depth_edge_length(Ntip = length(tree$tip.label), Nnode = tree$Nnode, 
      edge = z$edge, Nedge = nrow(tree$edge)[1], edge.length = z$edge.length)
    # Keep only the tips, and add the tip labels in case `z` order differs from
    # `tree`
  ...
  distlist <- foreach(i = spn, .packages = "phyloseq") %dopar% {
    ...
    if (weighted) {
      ...
      # if not-normalized weighted UniFrac, just return 'numerator'; the u-value in the
      # w-UniFrac description
      if (!normalized) {
        return(numerator)
      } else {
      ...
      # Unweighted UniFrac Subset matrix to just columns A and B
      edge_occ_AB <- edge_occ[, c(A, B)]
      # Keep only the unique branches. Sum the lengths
      edge_uni_AB_sum <- sum((tree$edge.length * edge_occ_AB)[rowSums(edge_occ_AB, 
        na.rm = TRUE) < 2, ], na.rm = TRUE)
      # Normalize this sum to the total branches among these two samples, A and B
      uwUFpairdist <- edge_uni_AB_sum/sum(tree$edge.length[rowSums(edge_occ_AB, 
        na.rm = TRUE) > 0])
      return(uwUFpairdist)
    }
  }
  ...
  # Take care of edge case where there are two samples -> 1 pair of indices ->
  # rbind doesn't return a matrix
  if (!is.matrix(matIndices)) 
    matIndices <- matrix(matIndices, ncol = 2)
...
gUniFrac <- function(physeq, alpha = 0.5) {
  ...
  unifracs <- GUniFrac::GUniFrac(OTU, tree=tree, alpha=alpha)$unifracs
  #UniFracMat <- unifracs[, , "d_UW"]       # Unweighted UniFrac
  #UniFracMat <- unifracs[, , "d_0"]        # GUniFrac with alpha 0
  #UniFracMat <- unifracs[, , "d_0.5"]      # GUniFrac with alpha 0.5
  #UniFracMat <- unifracs[, , "d_1"]        # Weighted UniFrac
  #UniFracMat <- unifracs[, , "d_VAW"]      # Variance adjusted weighted UniFrac
  UniFracMat <- unifracs[, , paste("d", alpha, sep="_")]
  return(as.dist(UniFracMat))
}
