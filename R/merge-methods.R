...
merge_phyloseq <- function(...) {
  ...
  for (i in unique(names(comp.list))) {
    # i='tax_table' check if length 1, if so, cat to merged.list.
  ...
  # Remove names to avoid any conflicts with phyloseq(), which does not need
  # named-arguments
...
#' @rdname merge_phyloseq_pair-methods
setMethod("merge_phyloseq_pair", signature("otu_table", "otu_table"), function(x, 
  y) {
  ...
  newx <- matrix(0, nrow = length(new.sp.names), ncol = length(new.sa.names), dimnames = list(new.sp.names, 
    new.sa.names))
...
#' @rdname merge_phyloseq_pair-methods
setMethod("merge_phyloseq_pair", signature("taxonomyTable", "taxonomyTable"), function(x, 
  y) {
...
#' @rdname merge_phyloseq_pair-methods
setMethod("merge_phyloseq_pair", signature("sample_data", "sample_data"), function(x, 
  y) {
...
#' @rdname merge_phyloseq_pair-methods
setMethod("merge_phyloseq_pair", signature("XStringSet", "XStringSet"), function(x, 
  y) {
  if (class(x) != class(y)) {
...
merge_taxa.indices.internal = function(x, eqtaxa, archetype) {
  ## If eqtaxa or archetype are character, interpret them to be OTUs and coerce them
  ## to integer indices
  if (is.character(archetype)) {
    ...
    # Else archetype is the numeric index of the eqtaxa that should be kept.  Need to
    # grab from unmodifed eqtaxa, and then decide
  ...
  # keepIndex is index of the OTU that is kept / everything merged into.  It must
  # be among the set of indices in eqtaxa or there is a logic error. Stop.
  if (length(keepIndex) <= 0L) {
    stop("invalid archetype provided.")
  }
  if (!keepIndex %in% eqtaxa) {
    stop("invalid archetype provided. It is not part of eqtaxa.")
  }
...
#' @rdname merge_taxa-methods
setMethod("merge_taxa", "phyloseq", function(x, eqtaxa, archetype = eqtaxa[which.max(taxa_sums(x)[eqtaxa])]) {
...
#' @rdname merge_taxa-methods
setMethod("merge_taxa", "otu_table", function(x, eqtaxa, archetype = eqtaxa[which.max(taxa_sums(x)[eqtaxa])]) {
  ...
  # Merge taxa by summing all the equivalent taxa and assigning to the one in
  # keepIndex
  if (taxa_are_rows(x)) {
...
setMethod("merge_taxa", "phylo", function(x, eqtaxa, archetype = 1L) {
  ...
  indList = merge_taxa.indices.internal(x, eqtaxa, archetype)
  removeIndex = indList$removeIndex
  # If there is too much to merge (tree would have 1 or 0 branches), return
  # NULL/warning
...
setMethod("merge_taxa", "XStringSet", function(x, eqtaxa, archetype = 1L) {
  ...
  indList = merge_taxa.indices.internal(x, eqtaxa, archetype)
  removeIndex = indList$removeIndex
  # If there is too much to merge (refseq would have 0 sequences), return
  # NULL/warning
...
setMethod("merge_taxa", "taxonomyTable", function(x, eqtaxa, archetype = 1L) {
  ...
  removeIndex = indList$removeIndex
  keepIndex = indList$keepIndex
  # # # Taxonomy is trivial in ranks after disagreement among merged taxa # # Make
  # those values NA_character_
  taxmerge <- as(x, "matrix")[eqtaxa, ]
  bad_ranks <- apply(taxmerge, 2, function(i) {
    length(unique(i)) != 1
  })
...
setMethod("merge_samples", signature("sample_data"), function(x, group, fun = mean) {
  ...
  # Remove any non-coercable columns.  Philosophy is to keep as much as possible.
  # If it is coercable at all, keep.  Coerce all columns to numeric matrix
  ...
  # get rownames from the 'group' column (always first) rownames(outdf) <-
  # as.character(outdf[, 1])
...
setMethod("merge_samples", signature("phyloseq"), function(x, group, fun = mean) {
  ...
  ### Add to build-call-list the remaining components, if present in x.  NULL is
  ### returned by accessor if object lacks requested component/slot.  Order of
  ### objects in list doesn't matter for phyloseq.  The list should not be named.
  if (!is.null(access(x, "tax_table"))) {
    phyloseqList <- c(phyloseqList, list(tax_table(x)))
  }
  if (!is.null(access(x, "phy_tree"))) {
    phyloseqList <- c(phyloseqList, list(phy_tree(x)))
  }
