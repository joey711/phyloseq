...
ordinate = function(physeq, method = "DCA", distance = "bray", formula = NULL, ...) {
  # If `physeq` is a formula, post deprecated notice, attempt to convert and
  # dispatch
  ...
  # # Start with methods that don't require # additional distance calculation.
  # (distance argument ignored) DCA
  if (method == "DCA") {
  ...
  # # Now resort to methods that do require a separate distance/dist-calc Define
  # ps.dist. Check the class of distance argument is character or dist
  if (inherits(distance, "dist")) {
    ps.dist <- distance
  } else if (class(distance) == "character") {
    # There are some special options for NMDS/metaMDS if distance-method is supported
    # by vegdist, so check first. If not, just calculate distance
...
DPCoA <- function(physeq, correction = cailliez, scannf = FALSE, ...) {
  ...
  if (!class(physeq) == "phyloseq") {
    stop("physeq must be phyloseq-class")
  }
  ...
  # if the patristic distances are not Euclidean, then correct them or throw
  # meaningful error.
...
################################################################################ vegan::cca 'extension'.  formula is main input to this function. This
################################################################################ complicates signature handling.  A new method with a separate name is defined
################################################################################ instead.  Must transpose the phyloseq otu_table to fit the vegan::cca
################################################################################ convention Whether-or-not to transpose needs to be a check, based on the
...
#' @rdname cca-rda-phyloseq-methods
setMethod("cca.phyloseq", signature = c("phyloseq", "formula"), function(physeq, 
  formula, method = "CCA", ...) {
  ...
  # Note that ade4 also has a conflicting 'cca' function.  You don't import
  # ade4::cca to avoid the conflict.
  if (method == "CCA") {
...
#' @rdname cca-rda-phyloseq-methods
setMethod("cca.phyloseq", signature = "otu_table", function(physeq, formula = NULL, 
  method = "CCA", ...) {
...
#' @rdname cca-rda-phyloseq-methods
setMethod("cca.phyloseq", signature = c("phyloseq", "NULL"), function(physeq, formula, 
  method = "CCA", ...) {
  # Absence of a formula (NULL) indicates unconstrained ordination.  Access
  # otu_table, and dispatch.
...
gapstat_ord = function(ord, axes = c(1:2), type = "sites", FUNcluster = function(x, 
  k) {
...
################################################################################ Define an internal function for accessing and orienting the OTU table in a
################################################################################ fashion suitable for vegan functions @keywords internal
...
#' @rdname capscale-phyloseq-methods
setMethod("capscale.phyloseq", c("phyloseq", "formula", "dist"), function(physeq, 
  formula, distance, ...) {
...
#' @rdname capscale-phyloseq-methods
setMethod("capscale.phyloseq", c("phyloseq", "formula", "character"), function(physeq, 
  formula, distance, ...) {
  data = data.frame(sample_data(physeq), stringsAsFactors = FALSE)
  # The goal here is to process the distance identifier string and dispatch
  # accordingly.
  ...
  if (!distance %in% unlist(distance("list"))) {
    # distance must be among the supported distance options (which is a superset of
    # vegdist).
