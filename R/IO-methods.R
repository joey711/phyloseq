...
#'  produced by \emph{mothur}.
#' @param mothur_list_file (Required).
...
#' 
#' @keywords internal
import_mothur_constaxonomy = function(mothur_constaxonomy_file, mothur_list_file, cutoff=NULL, parseFunction=parse_taxonomy_mothur){
  ...
  if( identical(parseFunction, parse_taxonomy_mothur) ){
  ...
  OTUids = create_otu_dict(mothur_list_file, cutoff)     
  if(length(names(OTUids)) != nrow(rawtab)){
    stop("Make sure you use the taxonomy file corresponding to the selected cutoff")
  }  
  names(taxlist) <- names(OTUids)
  return(build_tax_table(taxlist))
}
################################################################################
#' Creating mothur OTU ID file
#' mothur is naming the OTUs as Otu01, Otu02, etc.. In order to match the names 
#' generated by phyloseq, some files have to be renamed (for example 
#' constaxonomy files).
#' @param mothur_list_file
#' @param cutoff (Optional)
create_otu_dict <- function(mothur_list_file, cutoff=NULL){
  cutoffs = show_mothur_cutoffs(mothur_list_file)
  cutoff = select_mothur_cutoff(cutoff, cutoffs)
  # Read only the line corresponding to that cutoff  
  inputline = which(cutoffs == cutoff)
  rawlines = scan(mothur_list_file, "character", sep="\t", skip=(inputline-1), nlines=1, na.strings="", quiet=TRUE)
  lableline = scan(mothur_list_file, "character", sep="\t", nlines=1, na.strings="", quiet=TRUE)
  rawlines = rawlines[!is.na(rawlines)]
  # The first two elements are the cutoff and the number of OTUs. skip, and read to first comma for OTUnames  
  OTUnames = scan(text=rawlines, what="character", comment.char=",", quiet=TRUE)[3:length(rawlines)]
  OTUids = scan(text=lableline, what="character", comment.char=",", quiet=TRUE)[3:length(rawlines)]  
  names(OTUids) <- OTUnames  
  return(OTUids) 
}
################################################################################
...
import_mothur <- function(mothur_list_file=NULL,  mothur_group_file=NULL,
  ...
  mothur_shared_file=NULL, mothur_constaxonomy_file=NULL, parseFunction=parse_taxonomy_mothur){
  ...
  if( !is.null(mothur_constaxonomy_file) ){
    if( !is.null(mothur_list_file)){
      tax <- import_mothur_constaxonomy(mothur_constaxonomy_file, mothur_list_file, cutoff, parseFunction)
      pslist = c(pslist, list(tax))
    }
    else{
      stop("You have to additionally provide a mothur_list_file, because of naming issues.")
    }
  }  
...
#' @usage parse_taxonomy_mothur(char.vec)
...
#' @export
parse_taxonomy_mothur = function(char.vec){
  # Remove any leading empty space
  char.vec = gsub("^[[:space:]]{1,}", "", char.vec)
  # Remove any trailing space
  char.vec = gsub("[[:space:]]{1,}$", "", char.vec)
  if( length(char.vec) == 6 ){
    #Apply names according to mothur taxonomy
    names(char.vec) = c("Kingdom", "Phylum", "Class", "Order", "Family", "Genus")
  } else {
    warning("Empty taxonomy vector encountered.")
  }
  return(char.vec)
